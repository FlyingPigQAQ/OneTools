...
**FFmpeg binary must be present for the audio converter to work.** Place the static binary at `resources/ffmpeg/<arch>/ffmpeg` (e.g. `resources/ffmpeg/arm64/ffmpeg`). `src/main/utils/paths.ts` resolves this; `src/main/services/ffmpegManager.ts` tries the bundled binary first (`<process.arch>` then the other macOS arch, see `getFfmpegArchCandidates`), then falls back to `ffmpeg` on `$PATH`. If absent, conversions fail with `ENOENT`. Get them with `./scripts/fetch-ffmpeg-macos.sh` (arm64 + x64, ffmpeg + ffprobe), which downloads the pinned eugeneware/ffmpeg-static release and **verifies each file against a SHA-256 hardcoded in the script**. On macOS it also checks `otool`/`lipo`. The binaries are NOT committed — `resources/ffmpeg/` is gitignored (~250 MB for both arches x both binaries); CI downloads and hash-verifies them itself. They must be self-contained (static): a Homebrew ffmpeg links `/opt/homebrew` dylibs, cannot run on machines without Homebrew, and cannot be notarized. Check with `otool -L` (only `/usr/lib` + `/System` deps allowed). Signing/notarization is mandatory in CI — see `.github/workflows/build-macos.yml`.
